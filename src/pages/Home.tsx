    ...
    <div className=" grid grid-cols-12 gap-16 mb-20  ">
      <div className="col-span-4 hidden sm:block border-[1px] bg-white border-[#e6e7eb] overflow-y-auto customshadow rounded-md shadow-sm h-[650px]  sticky top-20">
        <div className="h-[700px] px-8 pt-6">
          <p className="text-[28px] leading-[40px] font-bold">Explore by Tags</p>
          <div className="h-[600px] overflow-y-auto mt-6">
            <div className="h-[700px] pr-2">
              {
                tags.map((item,index)=>{
                  return(
                    <div className="flex mt-2 py-2 items-center justify-between px-3 cursor-pointer rounded-md border-gray border-[1px] hover:bg-gray" key={index}>
                      <p className="text-base font-[500] text-lightBlack">{item.name}</p>
                      <p className="p-2 border-gray border-[1px] rounded-md">{item.count}</p>
                    </div>
                  )
                })
              }
